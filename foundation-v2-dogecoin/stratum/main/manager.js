...
const Manager = function(config, configMain) {
  ...
  this.handleShare = function(jobId, client, submission) {
    ...
    if (!job.handleSubmissions([submission.extraNonce1, submission.extraNonce2, submission.nTime, submission.nonce])) {
      console.log('[POOL DUPLICATE] id=' + submission.id + ' job=' + jobId + ' extraNonce1=' + submission.extraNonce1 + ' extraNonce2=' + submission.extraNonce2 + ' nTime=' + submission.nTime + ' nonce=' + submission.nonce);
    ...
    if (job.target >= headerBigInt) {
      ...
      if (shareDiff / difficulty < 0.99) {
        if (client.previousDifficulty && shareDiff >= client.previousDifficulty) {
          ...
          console.log('[POOL REJECT] id=' + submission.id + ' job=' + jobId + ' extraNonce1=' + submission.extraNonce1 + ' extraNonce2=' + submission.extraNonce2 + ' nTime=' + submission.nTime + ' nonce=' + submission.nonce);
          console.log('[POOL REJECT] header=' + headerBuffer.toString('hex'));
          console.log('[POOL REJECT] scryptHash=' + headerHash.toString('hex'));
          console.log('[POOL REJECT] shareDiff=' + shareDiff + ' required=' + difficulty);
